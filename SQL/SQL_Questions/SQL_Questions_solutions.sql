...
-- 21. Write a query to calculate total revenue generated by each pizza size.

SELECT pizza_name_id , SUM(total_price) AS Total_revenue_each_pizza FROM pizza_sales
GROUP BY pizza_name_id
ORDER BY Total_revenue_each_pizza DESC;



-- 22. Write a query to display each order along with the total quantity of pizzas in that order. 
